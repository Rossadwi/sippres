...
class siswaldashboardcontroller extends Controller
{
    ...
    public function getdashboard()
    {
        ...
        $datapresverif = DB::select("SELECT COUNT(*) as total FROM prestasi WHERE isverif = 1 and id_siswa = ?;",[$idsiswa]);
        $dataaktifverif = DB::select("SELECT COUNT(*) as total FROM keaktifan WHERE isverif = 1 and id_siswa = ?;",[$idsiswa]);
        $datainfolomba = DB::select("SELECT COUNT(*) as total FROM `infolomba`;");
        $all = [
            'totalpresverif' => $datapresverif[0]->total,
            'totalaktifverif' => $dataaktifverif[0]->total,
            'totalinfolomba' => $datainfolomba[0]->total
        ];
        return view('app/siswa/dashboard', ['all'=>$all]);
    }
